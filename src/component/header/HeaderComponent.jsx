...
const HeaderComponent = ({
  ...
  buttons = [
    // { text: 'Login', type: 'ghost', href: 'https://demo.eduvisahub.com', target: '_blank' },
    {
      text: "Get Demo",
      type: "primary",
      href: "https://demo.eduvisahub.com",
      target: "_blank",
    },
  ],
  onButtonClick = () => {},
  ...
  const [activeNavItem, setActiveNavItem] = useState(
    navigationItems.findIndex((item) => item.isActive)
  );
  ...
  return (
    <>
      {/* Header Section */}
      <section
        className={`fixed top-0 left-0 right-0 z-500 bg-[#aa14b5] border-b-2 border-[rgba(230,232,233,0.36)] transition-all duration-300 ${
      ...
      >
        <header className='flex justify-between items-center px-6 py-4 text-white max-w-[90%] mx-auto'>
          ...
          >
            <span
              className={`w-6 h-0.5 bg-white mb-1.5 transition-all duration-300 rounded-sm ${
      ...
      {/* Mobile Menu Overlay */}
      <div
        className={`fixed top-0 left-0 w-full h-screen bg-[#aa14b5] z-50 p-8 transform transition-transform duration-300 ease-in-out lg:hidden ${
